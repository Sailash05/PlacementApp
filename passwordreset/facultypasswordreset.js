...
async function changePassword() {
    let mobileno = document.querySelector('#Register-no').value;
    ...
    if(password.trim() != confirmPassword.trim()) {
        ...
        try {
            const response = await fetch(domain + 'faculty/resetpassword', {
                method: 'PUT',
                headers: {
            ...
            if(response.status === 201) {
                ...
                showFailMessage("Failed",data.message,"Please Enter the Correct Mobile Number");
            }
            else if(!response.ok) {
